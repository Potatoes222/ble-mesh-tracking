...
bool bmt_scan_list_add(const uint8_t *uuid, const uint8_t *mac, uint8_t addr_type,
    ...
    for (int i = 0; i < s_count; i++)
        if (memcmp(s_entries[i].uuid, uuid, 16) == 0) return false;

    if (s_count >= BMT_MAX_SCAN_LIST) return false;

    memcpy(s_entries[s_count].uuid, uuid, 16);
    memcpy(s_entries[s_count].addr, mac, 6);
    s_entries[s_count].addr_type = addr_type;
    s_entries[s_count].oob_info  = oob_info;
    s_entries[s_count].used      = true;
    s_count++;
    return true;
}
...
void bmt_scan_list_print(void) {
    ...
    for (int i = 0; i < s_count; i++) {
        bool already = bmt_node_table_uuid_provisioned(s_entries[i].uuid);
        printf("  [%d] %-7s UUID: ", i, bmt_uuid_type_str(s_entries[i].uuid));
        bmt_print_uuid(s_entries[i].uuid);
        printf("\n        MAC : ");
        bmt_print_mac(s_entries[i].addr);
...
void bmt_scan_list_provision(void) {
    int count = 0;
    for (int i = 0; i < s_count; i++) {
        if (bmt_node_table_uuid_provisioned(s_entries[i].uuid)) {
            printf("[PROV] Skip [%d] already provisioned\n", i);
            continue;
        }
        esp_ble_mesh_unprov_dev_add_t dev = {0};
        memcpy(dev.uuid, s_entries[i].uuid, 16);
        memcpy(dev.addr, s_entries[i].addr, 6);
        dev.addr_type = s_entries[i].addr_type;
        dev.oob_info  = s_entries[i].oob_info;
        ...
        if (err == ESP_OK) {
            printf("[PROV] Provisioning [%d] %s...\n", i, bmt_uuid_type_str(s_entries[i].uuid));
            count++;
