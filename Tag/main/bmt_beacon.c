...
static void start_adv_random_interval(void)
{
    ...
    uint32_t ms = BMT_ADV_INTERVAL_MIN_MS
                + (esp_random() % (BMT_ADV_INTERVAL_MAX_MS
                                   - BMT_ADV_INTERVAL_MIN_MS + 1));
...
uint8_t  bmt_beacon_sequence(void)     { return s_sequence; }
bool     bmt_beacon_is_active(void)    { return s_adv_active; }
