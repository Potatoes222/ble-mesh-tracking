...
bool                     bmt_mqtt_is_connected(void) { return s_connected; }
esp_mqtt_client_handle_t bmt_mqtt_get_client(void)   { return s_client; }
void                     bmt_mqtt_note_published(void) { s_published++; }
...
void bmt_mqtt_enqueue_tag_report(const bmt_tag_report_t *report, const uint8_t *scanner_mac)
{
    ...
    item.report = *report;
    if (scanner_mac) { memcpy(item.scanner_mac, scanner_mac, 6); item.has_mac = true; }
    else              { memset(item.scanner_mac, 0, 6);          item.has_mac = false; }
...
void bmt_mqtt_start_worker(void)
{
    ...
    if (!s_queue) { ESP_LOGE(TAG, "MQTT queue create failed"); return; }
...
static void mqtt_event_handler(void *args, esp_event_base_t base, int32_t id, void *data)
{
    ...
    case MQTT_EVENT_DATA: {
        ...
        if (strstr(payload, "ota_scanner")) {
            if (bmt_ota_is_running()) ESP_LOGW(TAG, "[RPC] OTA already running");
            else { ESP_LOGI(TAG, "[RPC] OTA Scanner triggered"); bmt_ota_trigger_all_scanners(); }
        } else if (strstr(payload, "ota_relay")) {
            if (bmt_ota_is_running()) ESP_LOGW(TAG, "[RPC] OTA already running");
            else { ESP_LOGI(TAG, "[RPC] OTA Relay triggered"); bmt_ota_trigger_all_relays(); }
