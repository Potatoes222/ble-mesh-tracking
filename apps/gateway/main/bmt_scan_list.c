                       ...
                       uint8_t addr_type, uint16_t oob_info)
{
	...
	for (int i = 0; i < s_count; i++)
		if (memcmp(s_list[i].uuid, uuid, 16) == 0)
			return false;
	if (s_count >= BMT_MAX_SCAN_LIST)
		return false;
...
void bmt_scan_list_print(void)
{
	...
	for (int i = 0; i < s_count; i++)
	{
		...
		for (int b = 0; b < 16; b++)
			printf("%02X%s", s_list[i].uuid[b], b < 15 ? ":" : "");
		printf("\n        MAC : ");
		for (int b = 0; b < 6; b++)
			printf("%02X%s", s_list[i].addr[b], b < 5 ? ":" : "");
...
void bmt_scan_list_provision(void)
{
	...
	if (count == 0)
		printf("[PROV] Nothing to provision.\n");
}
