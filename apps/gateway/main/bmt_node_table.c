...
int bmt_node_table_capacity(void) { return BMT_MAX_NODES; }
...
void bmt_node_table_save(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "save: nvs_open failed: %s", esp_err_to_name(err)); return;
    }
    ...
    if (err != ESP_OK) {
        ...
        int count = 0;
        for (int i = 0; i < BMT_MAX_NODES; i++) if (s_nodes[i].used) count++;
...
void bmt_node_table_load(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "load: nvs_open failed: %s", esp_err_to_name(err)); return;
    }
...
void bmt_node_table_print(void)
{
    ...
    for (int i = 0; i < BMT_MAX_NODES; i++) {
        ...
        printf("  Type        : %s\n",
               s_nodes[i].is_relay ? "RELAY" : s_nodes[i].is_scan ? "SCAN" : "UNKNOWN");
        printf("  UUID        : "); print_uuid(s_nodes[i].uuid); printf("\n");
        printf("  MAC         : "); print_mac(s_nodes[i].mac);   printf("\n");
