...
int bmt_zone_track_capacity(void) { return BMT_MAX_TRACKED_TAGS; }

void bmt_zone_reset_all(void) { memset(s_tags, 0, sizeof(s_tags)); }

bmt_tag_track_t *bmt_zone_track_get(int idx)
{
    if (idx < 0 || idx >= BMT_MAX_TRACKED_TAGS) return NULL;
    return &s_tags[idx];
}
...
bmt_tag_track_t *bmt_zone_track_find(uint16_t tag_id)
{
    for (int i = 0; i < BMT_MAX_TRACKED_TAGS; i++)
        if (s_tags[i].active && s_tags[i].tag_id == tag_id) return &s_tags[i];
    return NULL;
}
