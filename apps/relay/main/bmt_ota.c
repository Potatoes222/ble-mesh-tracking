...
bool bmt_ota_is_triggered(void) { return s_ota_triggered; }
...
{
    nvs_handle_t h;
    if (nvs_open(BMT_RELAY_NVS_NAMESPACE, NVS_READWRITE, &h) != ESP_OK) return;
...
static bool check_and_clear_pending(void)
{
    nvs_handle_t h;
    uint8_t val = 0;
    if (nvs_open(BMT_RELAY_NVS_NAMESPACE, NVS_READWRITE, &h) != ESP_OK) return false;
...
static void ota_wifi_task(void *arg)
{
    ...
    if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
        printf("[OTA] netif init failed: %s\n", esp_err_to_name(err)); goto ota_fail;
    }
    ...
    if (err != ESP_OK) { printf("[OTA] wifi init: %s\n", esp_err_to_name(err)); goto ota_fail; }
    ...
    if (err == ESP_OK) {
        ...
        mark_pending();   /* báo THÀNH CÔNG sau khi reboot xong, không báo ở đây */
...
ota_fail:
    ...
    if (s_evt_any_id) { esp_event_handler_instance_unregister(WIFI_EVENT, ESP_EVENT_ANY_ID, s_evt_any_id); s_evt_any_id = NULL; }
    if (s_evt_got_ip) { esp_event_handler_instance_unregister(IP_EVENT, IP_EVENT_STA_GOT_IP, s_evt_got_ip); s_evt_got_ip = NULL; }
    esp_wifi_deinit();
    if (s_sta_netif) { esp_netif_destroy_default_wifi(s_sta_netif); s_sta_netif = NULL; }
    if (s_wifi_evgrp) { vEventGroupDelete(s_wifi_evgrp); s_wifi_evgrp = NULL; }
...
{
    if (s_ota_triggered) {
        ESP_LOGW(TAG, "[OTA] Already triggered");
        return;
    }
...
static void report_pending_task(void *arg)
{
    ...
    if (check_and_clear_pending()) {
        ESP_LOGI(TAG, "[OTA] Phat hien vua OTA xong va reboot thanh cong - bao cao ve Gateway");
