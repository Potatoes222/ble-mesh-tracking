...
typedef enum { PHASE_GAP_SCAN = 0, PHASE_MESH_PUB = 1 } radio_phase_t;
                              ...
                              esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
                ...
                if (field_type == 0xFF && field_len >= 10 &&
                    ...
                    if (recv_mac != calc_mac) {
                        ESP_LOGW(TAG, "[OTA] Beacon HMAC mismatch (got 0x%04x, expect 0x%04x)"
                                 " — ignoring, co the la gia mao", recv_mac, calc_mac);
                        break;
                    }
        ...
        if (bmt_ota_is_triggered()) break;
        if (s_phase != PHASE_GAP_SCAN) break;
        ...
        if (!parse_tag_payload(param->scan_rst.ble_adv,
                               param->scan_rst.adv_data_len, &payload)) break;
