...
{
    nvs_handle_t h;
    if (nvs_open(BMT_SCAN_NVS_NAMESPACE, NVS_READWRITE, &h) != ESP_OK) return;
...
static bool check_and_clear_pending(void)
{
    nvs_handle_t h;
    uint8_t val = 0;
    if (nvs_open(BMT_SCAN_NVS_NAMESPACE, NVS_READWRITE, &h) != ESP_OK) return false;
...
static void ota_wifi_task(void *arg)
{
    ...
    if (err != ESP_OK) { printf("[OTA] wifi init failed: %s\n", esp_err_to_name(err)); goto ota_fail; }
