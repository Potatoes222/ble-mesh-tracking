...
uint8_t bmt_scan_core_scanner_id(void) { return s_scanner_id; }
...
esp_err_t bmt_scan_core_init(void)
{
    ...
    if (err != ESP_OK) { ESP_LOGE(TAG, "BT init failed!"); return err; }

    err = bmt_mesh_init();
    if (err != ESP_OK) { ESP_LOGE(TAG, "Mesh init failed!"); return err; }

    err = bmt_scan_start();
    if (err != ESP_OK) { ESP_LOGE(TAG, "Scan start failed!"); return err; }
